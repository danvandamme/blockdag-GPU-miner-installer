...
set "_V="
for /f "tokens=2 delims==" %%V in ('findstr /r "^set .VERSION=" install-gpu-miner.bat') do if not defined _V set "_V=%%V"
set "VERSION=%_V:"=%"
if "%VERSION%"=="" (
    echo [Backup] WARNING: Could not read VERSION from install-gpu-miner.bat
) else (
...
set "BACKUP_DIR=C:\DATA\BlockDAG Installers\Versions\GPU Miner Installer\GPU Miner Installer - %VERSION%"
if exist "%BACKUP_DIR%" (
